---
aliases: ["Python Libraries", "Librerías Python"]
tags:
  - programacion/python
  - redes/conceptos
  - redes/protocolos
...
import datetime # librería necesaria

ahora = datetime.datetime.now() # Con esto nos da la fecha y hora actual

# Podemos imprimir las partes de una datetime que en este caso es ahora
year = ahora.year
mes = ahora.month
dia = ahora.day
hora = ahora.hour
...
time = datetime.time(14, 15, 15) # Nos permite imprimir la hora que queramos
...
> [!info] `datetime.datetime.now()` vs `datetime.datetime.utcnow()`
> `datetime.datetime.now()` devuelve la fecha y hora actual según el reloj del sistema local, y por defecto no incluye información de zona horaria (es *naive*). Para trabajar con zonas horarias, es recomendable usar `datetime.datetime.now(tz=timezone.utc)` o `datetime.datetime.utcnow()` (que devuelve la hora UTC *naive*) y luego convertirla a una zona horaria específica usando el módulo `pytz` o `zoneinfo` (a partir de Python 3.9).

> [!tip] Uso de `timedelta` para aritmética de fechas
> El tipo `timedelta` es fundamental para realizar operaciones aritméticas con fechas y horas. Permite sumar o restar periodos de tiempo (días, segundos, microsegundos, etc.) a objetos `date`, `time` o `datetime`. Por ejemplo, `ahora + datetime.timedelta(days=7)` sumaría una semana a la fecha actual.
...
matches = re.findall("gato", text) # Esto nos permite listar y guardar en una lista las veces que aparece la palabra

text2 = "Hoy estamos en fecha 10/10/2023, mañana estaremos a 11/10/2023"

matches = re.findall("\\d{2}\\/\\d{2}\\/\\d{4}", text2) # Esto no es más que un patrón de búsqueda que indica: "trae `\\d{2}` (dos dígitos), seguido de `\\/` (una barra escapada), luego otros dos dígitos, y finalmente cuatro dígitos".

text3 = "Los usuario pueden contactarnos a soporte@danystarrkk.ecuador o a soporte@andresestrella26.tech"

matches = re.findall("(\\w+)@(\\w+\\.\\w{2,})", text3) # el `\\w` representa caracteres alfanuméricos (y guion bajo), mientras que `\\w+` representa uno o más de estos caracteres.
# por último definimos lo que es `\\w{2,}`: caracteres alfanuméricos con un mínimo de 2 caracteres y un máximo no definido, para no limitar la longitud del dominio de nivel superior (TLD).

text4 = "Mi gato esta en el tejado y mi perro esta en el jardín"
nuevo_texto = re.sub("gato", "perro", text) # Por defecto, re.sub realiza una sustitución de todas las ocurrencias no superpuestas. Para sustituir solo la primera coincidencia, se puede usar re.sub("gato", "perro", text, count=1).

text5 = "Campo1,Campo2,Campo3,Campo4,Campo5" # Nos permite a través de un delimitador crear una cadena de diferentes valores
nuevo_texto = re.split(",", text5)

print(nuevo_texto)
```

> [!info] Diferencias entre `re.findall()`, `re.search()` y `re.match()`
> *   `re.findall(pattern, string)`: Devuelve una lista de *todas* las coincidencias no superpuestas del patrón en la cadena.
> *   `re.search(pattern, string)`: Escanea la cadena buscando la *primera* ubicación donde el patrón produce una coincidencia. Devuelve un objeto `Match` si encuentra una, `None` en caso contrario.
> *   `re.match(pattern, string)`: Verifica si el patrón coincide al *principio* de la cadena. Devuelve un objeto `Match` si coincide, `None` en caso contrario. Es menos flexible que `re.search` para búsquedas generales.

> [!tip] La importancia de las *raw strings* (`r"..."`) en expresiones regulares
> Las *raw strings* son cruciales en Python al definir expresiones regulares. Al prefijar una cadena con `r` (ej. `r"\\d"`), Python no procesa las secuencias de escape de la barra invertida (`\`). Esto evita que caracteres como `\n` (salto de línea) o `\t` (tabulación) sean interpretados por Python antes de que la expresión regular los procese, asegurando que `\\d` se interprete como el patrón de un dígito y no como un error o una secuencia de escape de Python.
...
def validar_correo(correo):
    ...
    # La `r` (raw string) se usa para que Python interprete las barras invertidas `\` literalmente, lo cual es crucial en expresiones regulares para evitar conflictos con las secuencias de escape de Python. `\b` indica un límite de palabra.
    patron = r"\\b[A-Za-z0-9._+-]+@[A-Za-z0-9]+\\.[A-Za-z]{2,}\\b" # Esto nos permite crear un patrón que indica: `[caracteres alfanuméricos, puntos, guiones, etc.]` seguido de `@`, luego `[caracteres alfanuméricos]` y `\\.` (un punto escapado), y finalmente `[caracteres alfabéticos]{2,}` (dos o más caracteres alfabéticos para el TLD).

    
    if re.findall(patron, correo): # En este punto es en el cual se realiza la comparación y nos devuelve una lista de coincidencias. Si la lista no está vacía (es decir, hay coincidencias), la condición es `True`.
        return True
...
texto = "Hoy estamos a día 10/10/2023 y mañana estaremos a 11/10/2023"
...
for match in re.finditer(patron, texto):
    ...
    #tanto match.start como match.end nos permite identificar en donde comienza la cadena y en donde termina, el valor del indice donde comienza
```

> [!info] Eficiencia de `re.finditer()` para grandes volúmenes de texto
> A diferencia de `re.findall()`, que devuelve una lista completa de todas las coincidencias (lo que puede consumir mucha memoria para textos grandes), `re.finditer()` devuelve un iterador. Esto significa que las coincidencias se generan una a una a medida que se solicitan, lo que lo hace más eficiente en términos de memoria para procesar grandes volúmenes de texto o cuando solo se necesita procesar las coincidencias de forma secuencial.
...
if not os.path.exists("mi_directorio/mi_subdirectorio"): # Comprobamos su existencia
    os.makedirs("mi_directorio/mi_subdirectorio") # Creamos los directorios anidados

### Listando los recursos de una ruta
...
if os.path.exists("mi_directorio1"):
    os.rmdir("mi_directorio1") # solo podemos eliminar los directorios vacíos en el sistema.
...
> [!tip] `os.mkdir()` vs `os.makedirs()`
> *   `os.mkdir(path)`: Crea un único directorio. Fallará si el directorio padre no existe.
> *   `os.makedirs(path, exist_ok=False)`: Crea directorios recursivamente. Si `exist_ok` es `True`, no lanzará un error si el directorio ya existe. Es la opción preferida para asegurar que una estructura de directorios esté presente.

> [!warning] Funciones de eliminación y sus riesgos
> *   `os.remove(path)`: Elimina un archivo.
> *   `os.rmdir(path)`: Elimina un directorio *vacío*. Fallará si el directorio contiene archivos o subdirectorios.
> *   `shutil.rmtree(path)`: Elimina un directorio y *todo su contenido* (archivos y subdirectorios). Esta función es muy potente y debe usarse con extrema precaución, ya que no hay confirmación y la eliminación es irreversible.

> [!info] `os.path` para compatibilidad entre sistemas operativos
> El submódulo `os.path` es esencial para construir y manipular rutas de archivos de manera independiente del sistema operativo. Funciones como `os.path.join()`, `os.path.basename()`, `os.path.dirname()`, y `os.path.split()` manejan automáticamente los separadores de ruta (`/` en Linux/macOS, `\` en Windows), asegurando que tu código funcione correctamente en diferentes entornos.
...
ruta = os.path.join("mi_directorio", "mi_archivo.txt") # para pasarle esto debemos seguir la estructura [ruta, archivo final]
...
directorio = os.path.dirname(ruta) # Nos almacena el directorio que contiene
print(f"\n[+] Nombre del directorio: {directorio}\n")

directorio , archivo = os.path.split(ruta) # Con esto igualamos las variables a través de la tupla que se genera.
...
> [!info] Overhead de TCP vs UDP
> *   **TCP**: Tiene un *overhead* mayor debido a su mecanismo de establecimiento de conexión (three-way handshake), control de flujo, control de congestión, retransmisión de paquetes y verificación de errores. Esto lo hace más lento pero fiable.
> *   **UDP**: Tiene un *overhead* mínimo, ya que simplemente envía datagramas sin establecer una conexión, verificar la entrega o el orden. Esto lo hace más rápido y eficiente para aplicaciones sensibles a la latencia, pero a costa de la fiabilidad.
...
> [!tip] Parámetros de `socket.socket()`
> *   `socket.AF_INET`: Especifica la familia de direcciones IPv4. Para IPv6, se usaría `socket.AF_INET6`.
> *   `socket.SOCK_STREAM`: Indica que se usará el protocolo TCP (orientado a flujo de bytes, fiable).
> *   `socket.SOCK_DGRAM`: Indica que se usará el protocolo UDP (orientado a datagramas, no fiable).
...
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # este es una especie de descriptor el cual nos va a permitir aceptar conexiones entrantes y también nos permite
...
# que vamos a estar trabajando con conexiones del tipo TCP

server_address = ('localhost', 1234) # definimos el server_address a través de una tupla en donde describimos equipo local (`localhost`) y el puerto (1234)
server_socket.bind(server_address) # Con ayuda de bind, asociamos el socket a la dirección y puerto especificados.

server_socket.listen(1) # en este punto limitamos la cola de espera de conexiones pendientes a 1, lo que significa que el servidor solo aceptará una conexión a la vez.

while True:

    client_socket, client_address = server_socket.accept() # Al aceptar la conexión este debe devolvernos dos partes: el client_socket con el cual nos permite comunicarnos con el cliente
    # y el client_address que es una variable a través de la cual vamos a percibir la IP del cliente que se conecta con nosotros y el puerto aleatorio que se abre la conexión.

    data = client_socket.recv(1024) # Esta función nos permite recibir el mensaje del cliente que se conectó y debemos definir el tamaño de la información en bytes, por esto
    # es el 1024 que se define.

    print(f"\n[+] Mensaje recibido del cliente: {data.decode()}")
    print(f"[+] Información del cliente que se ha comunicado con nosotros: \n {client_address}")

    client_socket.sendall(f"Un saludo crack\n".encode()) # Nosotros en este punto logramos enviar un mensaje haciendo uso del client_socket que se consiguió al entablar la conexión

    client_socket.close()
```

> [!info] El *Three-Way Handshake* en TCP (lado del servidor)
> 1.  `socket.bind((host, port))`: Asocia el socket a una dirección IP y un puerto específicos en el servidor.
> 2.  `socket.listen(backlog)`: Pone el socket en modo de escucha, esperando conexiones entrantes. El `backlog` define el número máximo de conexiones pendientes que el sistema operativo puede mantener en cola.
> 3.  `socket.accept()`: Bloquea la ejecución hasta que un cliente se conecta. Cuando una conexión es aceptada, devuelve un *nuevo socket* (`client_socket`) dedicado a la comunicación con ese cliente específico y la dirección del cliente (`client_address`). El `server_socket` original permanece en escucha para nuevas conexiones.
...
server_address = ("localhost", 1234) # definimos en una tupla los parámetros del servidor a conectarse
client_socket.connect(server_address) # con esto establecemos una conexión con el servidor.

try:
    message = b"Este es un mensaje de prueba que envio al servidor" # Generamos un mensaje en bytes
    client_socket.sendall(message) # Enviamos el mensaje al servidor
    data =  client_socket.recv(1024)

    print(f"[+] El servidor nos ha respondido con un: {data.decode()}") #Imprimimos el mensaje sin olvidarnos del decode para que no esté en formato bytes

finally: # recordemos que finally se ejecuta sin importar lo que pase en try es por eso que lo usamos para cerrar el socket
    client_socket.close()
...
import socket #importamos socket para poder jugar con puertos
...
def start_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s: # De esta forma al igual que con archivos el socket o puerto se cerrará solo
        s.bind((host, port)) # Recordemos que debemos pasarle una tupla a nuestro bind para quedar en escucha.
        print(f"\n[+] Servidor en escucha en {host} por el puerto {port}")
        s.listen(1) # Nos permite definir el número de conexiones en simultáneo
        conn, addr = s.accept() # Recordemos que el accept nos devuelve dos valores: el socket y la dirección.

        with conn: # una vez salgamos del with se cierra el descriptor de archivo
            print(f"\n[+] Se ha conectado un nuevo cliente: {addr}")
            while True:
                data = conn.recv(1024) # nos permite recibir mensajes y además debemos definir lo que es el tamaño máximo a recibir
                if not data: # en pocas palabras decimos que si el cliente no nos envía nada salimos del bucle
                    break
...
def start_udp_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s: # En el caso del tipo de conexión UDP usamos SOCK_DGRAM.
        ...
        while True:
            data, addr = s.recvfrom(1024) # Como UDP no está totalmente orientado a conexiones no es necesario entablar un three-way handshake.
            # Es por eso que no hacemos uso del accept y del listen. Con recvfrom nos devuelve los mensajes transmitidos y en addr devuelve la información como el host y port
...
def start_udp_client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s: # Realizamos esto para que la conexión se cierre al terminar
        # s.sendto(b"Hola aqui esta el cliente", (host, port)) # Jugamos con sendto porque nos permite enviar el mensaje y la tupla del host y port
        # Los mensajes con tildes van a causar un error y para solucionar esto vamos a tener que definir de la siguiente manera:

        mensaje ="Hola aquí está el cliente".encode("utf-8") # de esta forma es una buena práctica realizar para poder enviar tildes y caracteres no ASCII
...
> [!tip] Opciones comunes de `setsockopt()`
> Además de `SO_REUSEADDR`, otras opciones útiles incluyen:
> *   `SO_KEEPALIVE`: Habilita el envío de mensajes *keep-alive* en conexiones TCP para detectar si el cliente sigue activo.
> *   `SO_LINGER`: Controla el comportamiento del cierre del socket cuando hay datos pendientes en el búfer.
> *   `SO_BROADCAST`: Permite enviar mensajes de difusión (broadcast) en sockets UDP.
...
class ClientThread(threading.Thread): # creamos la clase y hacemos uso de herencia para poder usar métodos y propiedades de la original
    ...
    # Este es el método que sobrescribimos
    def run(self):

        message = ""

        while True:
            data = self.client_sock.recv(1024) # recibimos el mensaje
            message = data.decode() # le hacemos un decode para verlo en formato normal, recordemos que cuando hacemos el decode se arrastra un salto de línea

            #pdb.set_trace() # Breakpoint Esto nos permite frenar el programa y ver los valores

            if message.strip() == "bye": # establecemos que si el cliente ingresa un bye se termine la conexión osea el bucle, usamos strip para eliminar el salto de línea que nos agrega decode
                break

            print(f"[+] Mensaje enviado por el cliente: {message.strip()}") # Traza para ver que el mensaje del cliente
            self.client_sock.send(data) # enviamos lo mismo que nos envía el cliente
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # Con esto podemos manipular un nivel a través de 3 campos donde (nivel, propiedad, igualdad)
    # la propiedad de SO_REUSEADDR nos permite reutilizar el estado de espera y en vez de esperar nos permite utilizarla.
    server_socket.bind((HOST, PORT))

    print(f"[+] En espera de conexiones entrantes")
    
    while True:
        server_socket.listen() # Pone el socket en modo de escucha. Aunque se usen hilos, es necesario llamar a listen() para que el servidor acepte conexiones. El argumento opcional 'backlog' (ej. listen(5)) define el número máximo de conexiones pendientes en cola.
        ...
        new_thread.start() # Esto nos permite llamar a un método run pero lo vamos a sobrescribir en la clase
```
...
def start_client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            data = s.recv(1024)
            print(f"[+] El mensaje de respuesta es: {data.decode()} ")
...
> [!warning] El GIL (Global Interpreter Lock) y el *threading* en Python
> El Global Interpreter Lock (GIL) en CPython (la implementación estándar de Python) permite que solo un hilo nativo ejecute bytecode de Python a la vez, incluso en sistemas multi-core. Esto significa que los hilos de Python no logran paralelismo real para tareas *CPU-bound*.
> Sin embargo, para tareas *I/O-bound* (como operaciones de red o lectura/escritura de archivos), el GIL se libera durante las operaciones de E/S, permitiendo que otros hilos de Python se ejecuten mientras uno espera por E/S. Por esta razón, el *threading* sigue siendo muy efectivo para mejorar la concurrencia en aplicaciones de red en Python.
...
def star_chart_server():
    ...
    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # Generamos la conexión tipo TCP
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)# es una especie de TIME_WAIT
    ...
    while True:
        ...
        client_message = client_socket.recv(1024).strip().decode() # Recibimos el mensaje y podemos quitar el salto de línea y de una vez también decodearlo
    ...
    client_socket.close() # Cerramos el servidor una vez que termina el bucle
...
def start_chat_client():
    ...
    while True:
        ...
        client_socket.send(client_message.encode()) # Recordemos que los mensajes tienen que estar en bytes

        if client_message == 'bye':
            break

        server_message = client_socket.recv(1024).strip().decode() # Recibimos el mensaje del servidor
        print(f"\n[+] Mensaje del servidor: {server_message}") # Imprimimos el mensaje del servidor
