---
aliases: [Tkinter, GUI Python, Desarrollo GUI]
...
> [!info]
> Tkinter es un *wrapper* de Python para el *toolkit* Tk. Esto significa que Python interactúa con las funciones de Tk, que a su vez maneja la representación gráfica y los eventos del sistema operativo. Su inclusión en la biblioteca estándar de Python lo convierte en una opción excelente para prototipado rápido y aplicaciones de escritorio ligeras.
...
> [!info]
> El `mainloop()` es el corazón de cualquier aplicación Tkinter. Es un bucle infinito que escucha y despacha eventos (clics, pulsaciones de teclas, redimensionamiento de ventana, etc.). Sin él, la ventana se crearía y se cerraría inmediatamente, ya que el programa Python terminaría su ejecución. Es un concepto fundamental en la programación de GUIs basada en eventos.
...
> [!tip]
> Los gestores de geometría `pack()`, `grid()` y `place()` son mutuamente excluyentes dentro de un mismo contenedor. Es decir, no se debe mezclar `pack()` y `grid()` en el mismo *frame* o ventana principal. Sin embargo, se pueden usar diferentes gestores en diferentes contenedores (por ejemplo, `pack()` en la ventana principal para un *frame*, y `grid()` dentro de ese *frame* para sus *widgets* internos).
...
def accion_botton():
    print(f"[+] Se ha presionado el botón")

root = tk.Tk() # Esto es necesario ya que es una especie de ventana principal

#root.title("Mi primera aplicación") # Este es el título de la ventana
...
label.pack() # Nos permite representar la información del label en la ventana
button.pack()

root.mainloop() # Este método nos permite ejecutar y manejar todos los eventos que se realizan en la aplicación
```
...
label3.pack(side=tk.LEFT, fill=tk.Y)  # Con ayuda de side podemos posicionar nuestros label, tenemos (LEFT, RIGHT, BOTTOM, TOP)
...
label1.grid(row=0, column=0) # Cuando hacemos uso de grid tenemos que especificar en una especie de tabla Excel en la cual se representa
...
> [!warning]
> El gestor de geometría `place()` ofrece un control absoluto sobre la posición y el tamaño de los *widgets*. Sin embargo, su uso puede dificultar la creación de interfaces responsivas que se adapten bien a diferentes tamaños de ventana o resoluciones de pantalla, ya que las posiciones absolutas no escalan automáticamente. Se recomienda para diseños muy específicos o cuando se necesita un control pixel a pixel.
...
root = tk.Tk()
root.geometry('800x400') # Esto nos permite definir el tamaño de la pantalla con la que se abrirá nuestro programa
...
label2.place(relx=0.8, rely=0.2) # Esto nos permite movernos de manera relativa, o sea que dependiendo del tamaño de la pantalla va a distanciar la información
label3.place(relx=0.5 , rely=0.5, anchor=tk.CENTER) # Con ayuda de anchor podemos centrar sin importar el tamaño de la ventana
...
> [!info]
> Los módulos `messagebox` y `filedialog` no son *widgets* en sí mismos, sino módulos que proporcionan funciones para interactuar con diálogos estándar del sistema operativo. Esto mejora la experiencia del usuario al ofrecer interfaces familiares para tareas comunes como guardar archivos o mostrar alertas, y son cruciales para aplicaciones GUI completas.
...
frame = tk.Frame(root, bg="blue", bd=5) # Frame es una especie de contenedor la cual no hace nada a menos que la usemos para contener algo, aquí definimos sus atributos
frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER) # En este punto definimos en dónde queremos que esté

label1 = tk.Label(frame, text="Esto es mi label1 de prueba", bg="green") # ya aquí usamos y decimos que nuestro label lo vamos a ubicar dentro de frame
label2 = tk.Label(frame, text="Label2", bg="red") # ya aquí usamos y decimos que nuestro label lo vamos a ubicar dentro de frame
...
canvas = tk.Canvas(root, width=500, height=500, bg="white" ) # El definir un canvas no es más que representar una especie de lienzo en el cual podemos representar diferentes cosas
canvas.pack(pady=20)

oval = canvas.create_oval(50,50, 150,150, fill="red") # Usamos una propiedad la cual nos permite crear círculos; se definen dos puntos (50,50) y (150,150) y el color
...
def accion_menu():
    messagebox.showinfo("Menú", "Se ha tensado la cosa") # Nos permite lanzar ventanas informativas, recibe dos parámetros: el título para la ventana y el mensaje

barra_menu = tk.Menu(root)
root.config(menu=barra_menu) # Le decimos que el menú o barra de menú principal va a ser barra_menu

menu1 = tk.Menu(barra_menu, tearoff=0) # Creamos ahora un nuevo menú pero que pertenece a barra_menu, mas no a root
# Con tearoff=0 eliminas las líneas que separaban a nuestras opciones
barra_menu.add_cascade(label="Menú", menu=menu1) # Tenemos que agregar nuestro menú, con label le damos un nombre

menu1.add_command(label="Opción1") # En este punto creamos opciones las cuales vamos a ver al presionar en el menú
menu1.add_command(label="Opción2")

menu2 = tk.Menu(barra_menu, tearoff=0) # Creamos un nuevo menú
barra_menu.add_cascade(label="Menú 2", menu=menu2)

menu2.add_command(label="Opción 1") # Agregamos opciones al menú
...
button = tk.Button(text="Enviar", command=enviar_mensaje)
button.pack()
...
> [!tip]
> La Programación Orientada a Objetos (POO) es fundamental para proyectos de GUI más complejos. Al encapsular la lógica y los *widgets* relacionados dentro de clases, se mejora la modularidad, la reutilización del código y la facilidad de mantenimiento. Cada instancia de una clase puede representar una ventana o un componente reutilizable, lo que simplifica la gestión de estados y eventos.
...
class SimpleTextEditor:
    def __init__(self, root):
        ...
        self.text_area.pack(fill=tk.BOTH, expand=1) # Con tk.BOTH y expand=1 podemos ocupar todo el espacio de la ventana
        self.current_open_file = ''
    
    def quit_confirm(self):
        if messagebox.askokcancel("Salir", "¿Estás seguro que deseas salir?"):
            self.root.destroy() # Con ayuda de la función destroy podemos eliminar, cerrar o destruir la ventana principal

    def open_file(self):
        filename = filedialog.askopenfilename() # Nos devuelve la ruta absoluta del archivo que escojamos
        ...
        if filename:
            ...
            if self.text_area.get("1.0", tk.END):
                ...
                if opt:
                    self.text_area.delete("1.0", tk.END) # Esta es una forma con la cual podemos eliminar el texto que tengamos en el área antes de abrir un archivo
    ...
    def new_file(self):
        if self.text_area.get("1.0", tk.END):
            opt = messagebox.askyesno("Eliminar", "Tienes información sin guardar. ¿Deseas crear un nuevo archivo?")
            if opt:
                self.text_area.delete("1.0", tk.END) # Esta es una forma con la cual podemos eliminar el texto que tengamos en el área antes de abrir un archivo
                self.current_open_file=''
    ...
    def save_file(self):
        if not self.current_open_file:
            filename = filedialog.asksaveasfilename() # Nos permite indicar la ruta y el nombre del archivo a guardar
...
menu_bar = tk.Menu(root) # Creamos nuestro menú principal
menu_options = tk.Menu(menu_bar, tearoff=0) # Creamos un submenú para las opciones

# Definimos las opciones para el menú
...
root.config(menu=menu_bar) # Configuramos que menu_bar va a ser el menú principal
...
> [!info]
> El desarrollo de una calculadora es un ejercicio clásico para aprender GUI, ya que involucra la gestión de eventos de botones, la actualización dinámica de una pantalla de texto (`tk.Entry`), y la implementación de lógica de procesamiento de expresiones. Es una excelente manera de practicar la interacción entre la interfaz y el *backend* lógico de la aplicación.
...
class Calculadora:
    ...
    def __init__(self, master): # A master la vamos a usar como la ventana principal cuando sea necesario
        self.master = master
        ...
        self.master.bind("<Key>", self.key_press) # Método especial que nos permite detectar las pulsaciones del teclado
    ...
    def build_button(self,button, row, col):
        if button == "C":
            b = tk.Button(self.master, text=button, width=3, command=lambda: self.clear_display()) # A través de la función lambda podemos evitar que se ejecute la función sin que sea llamada
        elif button == "=":
    ...
    def click(self, button): # Nos permite identificar los clics dentro de la GUI
    ...
    def operation(self): # Nos permite identificar las diferentes operaciones y ejecutarlas
...
> [!info]
> Este proyecto combina la programación de GUI con la programación de red y concurrencia. El uso de `threading` es crucial para que la interfaz de usuario permanezca responsiva mientras el cliente espera mensajes del servidor. `socket` proporciona la base para la comunicación de red, y `ssl` añade una capa de seguridad esencial para cifrar los datos transmitidos.
...
def client_thread(client_socket, clients, usernames):
    ...
    username = client_socket.recv(1024).decode() # Recibimos el usuario por parte del cliente y lo decodificamos
    usernames[client_socket] = username  # Tenemos el diccionario vacío llamado usernames; almacenaremos en este, como clave, el client_socket y, como valor, el nombre de usuario que recibimos

    print(f"\n[+] El usuario {username} se ha conectado al chat")

    for client in clients: # Iteramos a través de un bucle por todos los clientes almacenados
        if client is not client_socket: # Con este condicional comprobamos si el cliente no se encuentra en nuestra lista; esto ayuda a enviar este mensaje solo a usuarios entrantes y no repetidamente
            client.sendall(f"\n El usuario {username} ha entrado al chat\n\n".encode()) # Enviamos el mensaje al cliente

    while True:
        try:
            message = client_socket.recv(1024).decode() # Vamos a estar recibiendo la información
            if not message:
                break

            if message == "!usuarios": # Esto es útil para listar los usuarios conectados
                client_socket.sendall(f"\n [+] Listado de usuarios disponible: {','.join(usernames.values())}\n\n".encode()) # Con esto enviamos al cliente que lo solicitó la lista de usuarios; es por eso que no enviamos en un bucle para todos
                continue # Evitamos que el programa entre al for que tenemos debajo; con esto solo queda en un modo el cual espera la siguiente petición
            
            for client in clients: # Iteramos por la lista
                if client is not client_socket:  # Verificamos que no seamos nosotros mismos
                    client.sendall(f"{message}\n".encode()) # Enviamos el mensaje a los clientes sobre la conexión de un usuario

        except:
            break

    # Las siguientes líneas se ejecutan cuando se intenta enviar el mensaje a un usuario no conectado
    client_socket.close() # Cierra el socket
    clients.remove(client_socket) # Remueve el socket de la lista de clientes
    del usernames[client_socket] # Remueve el usuario del diccionario de usuarios
...
def main():
    ...
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # Definimos qué tipo de conexión queremos entablar
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # TIME_WAIT
    server_socket.bind((host, port)) # Con bind configuramos el socket para generar la conexión.
    server_socket = ssl.wrap_socket(server_socket, keyfile="server-key.key", certfile="server-cert.pem", server_side=True) # Cifrado SSL
    server_socket.listen() # Nos pone en escucha

    print(f"\n[+]El Servidor se encuentra en escucha de conexiones entrantes....")
    ...
    while True:
        ...
        client_socket, addres = server_socket.accept() # Aceptamos las conexiones y almacenamos el socket del cliente, al igual que la dirección (address) que contiene (IP, puerto)
        clients.append(client_socket) # Almacenamos el socket del cliente en nuestra lista de clientes

        print(f"\n[+] Se ha conectado: {addres}")

        thread = threading.Thread(target=client_thread, args=(client_socket, clients, usernames)) # Creamos un hilo y llamamos a la función client_thread con las variables que le pasamos a args
        thread.daemon = True # Es una forma de garantizar que, cuando se cierre el programa, nuestros hilos (threads) se cierren y no queden en segundo plano
        thread.start() # Iniciamos el hilo creado anteriormente
...
def send_message(client_socket, username, text_widget, entry_widget):
    ...
    client_socket.sendall(f"{username}> {message}".encode()) # Enviamos el mensaje a nuestro servidor

    entry_widget.delete(0, END) # Limpiamos la entrada de datos para poder seguir imprimiendo
    text_widget.configure(state='normal') # Habilitamos la escritura momentáneamente para representar el mensaje
    text_widget.insert(END, f"{username} > {message}\n") # Insertamos el mensaje en la pantalla de texto
    text_widget.configure(state='disable') # Deshabilitamos la escritura para que no se inserten mensajes no procesados

def recive_message(client_socket, text_widget):
    while True:
        try:
            message = client_socket.recv(1024).decode() # Usando el bucle infinito, vamos a estar recibiendo información continuamente.

            if not message:
                break

            text_widget.configure(state='normal') # Habilitamos la escritura momentáneamente para representar el mensaje
            text_widget.insert(END, message) # Insertamos el mensaje en la pantalla de texto
            text_widget.configure(state='disable') # Deshabilitamos la escritura para que no se inserten mensajes no procesados
...
def list_users_requests(client_socket):
    client_socket.sendall("!usuarios".encode()) # Envía una petición para que se regrese la lista de usuarios conectados

def exit_request(client_socket, username, window):
    client_socket.sendall(f"[!] El usuario {username} ha abandonado el chat \n\n".encode()) # Enviamos un mensaje a todos los usuarios de que cierto usuario abandonó el chat
    client_socket.close() # Cerramos el socket del usuario que dio en el botón salir

    window.quit() # Cerramos la ventana de ese usuario
    window.destroy() # Destruimos la ventana del usuario que dio en el botón salir
...
def main():
    ...
    client_socket.connect((host, port)) # Generamos la conexión con ayuda de connect a un servidor; es por eso que le pasamos una tupla con el host y el puerto

    username = input(f"\n[+] Introduce tu usuario: ")

    client_socket.sendall(username.encode()) # Enviamos el nombre de usuario a nuestro servidor para que nos reconozca

    window = Tk() # Con esto inicializamos nuestra ventana principal
    ...
    frame_widget.pack(fill=BOTH, expand=1, pady=5, padx=5) # Definimos la representación del frame con pack

    entry_widget = Entry(frame_widget) # Definimos el entry como parte del frame_widget
    entry_widget.bind("<Return>", lambda _: send_message(client_socket, username, text_widget, entry_widget)) # Con ayuda de bind detectamos pulsaciones y llamamos a una función lambda para que no se ejecute de manera automática
    entry_widget.pack(side=LEFT, fill=X, expand=1) # Representamos el entry en nuestra ventana principal

    button_widget = Button(frame_widget, text="Enviar", command=lambda: send_message(client_socket, username, text_widget, entry_widget)) # Generamos un botón y lo asociamos a nuestro frame
    ...
    thread = threading.Thread(target=recive_message, args=(client_socket, text_widget)) # Creamos un nuevo hilo y llamamos a una función, especificándole los argumentos que vamos a pasarle
    thread.daemon = True # Es una garantía para que los hilos no queden en espera o en segundo plano
